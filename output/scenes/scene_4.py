...
class Scene4(Scene):
    def construct(self):
        ...
        title = Text("Transition to Cloud Computing", font_size=40, color=BLUE, weight=BOLD).to_edge(UP, buff=0.5)
        self.play(DrawBorderThenFill(title), run_time=0.8)
        self.wait(0.3)

        # Process steps with enhanced styling
        step_data = [['Cloud Rise', 'Visualize clouds representing '], ['Transformation', 'Animate clouds morphing into c'], ['Scalability', 'Show arrows indicating scalabi'], ['Cost Efficiency', 'Illustrate cost benefits with '], ['Focus Shift', 'Depict developers focusing on ']]
