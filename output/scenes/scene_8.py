...
class Scene8(Scene):
    def construct(self):
        ...
        title = Text("The Role of Compilers and Abstractions in Dev", font_size=40, color=BLUE, weight=BOLD).to_edge(UP, buff=0.5)
        self.play(DrawBorderThenFill(title), run_time=0.8)
        self.wait(0.3)

        # Process steps with enhanced styling
        step_data = [['Write Code', 'Develop high-level code using '], ['Compile Code', 'Use a compiler to translate th'], ['Execute Program', 'Run the compiled program on ha']]
